    ...
    <div className="mx-2 my-2 h-[80px] shrink-0 rounded-2xl bg-card/70 backdrop-blur-xl ring-1 ring-white/10 shadow-card px-4 grid grid-cols-[1fr_auto_1fr] items-center gap-4">
      ...
      <div className="flex w-[min(720px,55vw)] flex-col items-center gap-1.5">
        ...
        <div className="flex w-full items-center gap-2">
          <span className="w-10 text-right text-[11px] tabular-nums text-muted-foreground">{fmt(position)}</span>
          <input
            type="range"
            min={0}
            max={duration || 0}
          ...
          />
          <span className="w-10 text-[11px] tabular-nums text-muted-foreground">{fmt(duration)}</span>
        </div>
      </div>
